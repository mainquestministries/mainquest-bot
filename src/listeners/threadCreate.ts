	...
	public override async run(thread: ThreadChannel) {
		...
		if (starter_msg?.author.id === this.container.client.user?.id) {
			...
			if (original_user === undefined) {
				await thread.send(
					'**Hinweis:** *Der ursprüngliche Nutzer befindet sich wahrscheinlich nicht mehr auf dem Server. Ist dies nicht so, bitte erwähne ihn selbst mit @nutzer um ihn in diesen Thread einzuladen'
				);
				return;
			}
